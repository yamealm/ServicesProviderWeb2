...
public class ListEgressWaitController extends GenericAbstractListController<Product> {
        ...
        try {
            ...
            if (producSeries != null && !producSeries.isEmpty()) {
                ...
                for (ProductSerie productSerie : producSeries) {
                    ...
                    item.appendChild(permissionEdit ? new ListcellEditButton("viewWait.zul", productSerie,Permission.EDIT_TRANSIT) : new Listcell());
                    item.appendChild(permissionDelete  ? new ListcellRemoveButton ("adminEgressInitWait.zul", productSerie,Permission.REMOVE_TRANSIT) : new Listcell());
    ...
    private void deleteProductSerie( Listitem listItem) {
        try {
            ...
            AccessControl.saveAction(Permission.REMOVE_WAIT, "product = " + productSerie.getProduct().getId() + " and product serie = " + productSerie.getSerie());
            loadData();
        } catch (Exception ex) {
            showError(ex);
        }
